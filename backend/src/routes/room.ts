...
export function createRoomRouter(io: Server) {
  ...
  router.post("/create", (req, res) => {
    ...
    if (!adminName)
      return res.status(400).json({ error: "adminName required" });
  ...
  router.post("/join", (req, res) => {
    ...
    if (!roomId || !memberName)
      return res.status(400).json({ error: "roomId and memberName required" });
  ...
  router.post("/ready", (req, res) => {
    ...
    console.log(`[Room] Stream ready: ${roomId}`);
